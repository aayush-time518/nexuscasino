...
function AppImage({
  ...
  // Use a reliable placeholder service if local file is missing
  const defaultFallback = 'https://placehold.co/400x300?text=No+Image';
  const effectiveFallback = fallbackSrc === '/assets/images/no_image.png' ? defaultFallback : fallbackSrc;

  const [imageSrc, setImageSrc] = useState(src || effectiveFallback);
  const [isLoading, setIsLoading] = useState(true);
  const [hasError, setHasError] = useState(false);

  useEffect(() => {
    setImageSrc(src || effectiveFallback);
    setHasError(false);
    setIsLoading(true);
  ...
  const handleError = () => {
    if (!hasError && imageSrc !== effectiveFallback) {
